class ProjectUsersController < ApplicationController
  ...
  before_action :check_manager_permission, only: [:get_available_users, :index, :create, :update, :destroy]
  ...
  def create
      ...
      if invalid_users.any?
        render json: { message: 'The following users have invalid roles (only developer or qa allowed)', 
                       users: invalid_users.map { |user| { id: user.id, user_type: user.user_type } } },
      ...
      render json: { 
        ...
        users: users.map { |user| { id: user.id, role: user.user_type } }
      }, status: :created
    end
